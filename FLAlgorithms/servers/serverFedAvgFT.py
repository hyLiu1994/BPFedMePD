...
from FLAlgorithms.users.userFedAvgFT import UserAVGFT
...
class FedAvgFT(Server):
    def __init__(self, model, times, args):
        ...
        for i in range(total_users):
            ...
            user = UserAVGFT(id, train, test, model, args)
    ...
    def train(self, add_new_client = False):
        loss = []
        for glob_iter in range(self.num_glob_iters - self.fineturn_iters):
        ...
        for glob_iter in range(self.fineturn_iters):
            print("-------------Fine Turn Round number: ",glob_iter + self.num_glob_iters - self.fineturn_iters, " -------------")
            self.evaluate()
            self.selected_users = self.select_users(glob_iter,self.num_users)
            for user in self.selected_users:
                user.train(self.local_epochs) #* user.train_samples 
